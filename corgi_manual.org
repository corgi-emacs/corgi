...
active in a given buffer.

~corkey/install-bindings~ takes two optional arguments, a list of binding files,
and a list of signal files, so ~(corgi/install-bindings)~ is really just a shorthand for

#+begin_src emacs-lisp
(corgi/install-bindings '(corgi-keys) '(corgi-signals))
#+end_src
...
Alternatively you can provide additional file references, in which case the
definitions in each will be combined. Files later in the list will override
definitions from files earlier in the list, so if you create a file in your
Emacs config directory called ~my-keys.el~, and initialize Corkey like this:
  
#+begin_src emacs-lisp
(corgi/install-bindings '(corgi-keys my-keys) '(corgi-signals))
#+end_src

Then you can override bindings or add new ones in ~my-keys.el~.
